...
const orderStatusClass: any = {
  ...
  cancelled: "bg-gray-100 text-gray-600 dark:bg-gray-700 dark:text-gray-300 border-transparent",
  disputed:  "bg-red-100 text-red-700 dark:bg-red-900/30 dark:text-red-300 border-transparent",
};
=======
...
<<<<<<< HEAD
  ...
  const [editForm, setEditForm] = useState({ name:"", username:"", email:"", adminRole:"support", password:"" });
...
<<<<<<< HEAD
  function openEdit(s: any) {
    if (s.isLocked) return show("حساب المالك محمي ولا يمكن تعديله", false);
    setEditing(s);
    setEditForm({ name: s.name || "", username: s.username || "", email: s.email || "", adminRole: s.adminRole || "support", password: "" });
  }
  async function saveEdit() {
    if (!editing) return;
    if (!editForm.name || !editForm.email) return show("الاسم والبريد مطلوبان", false);
    setEditSaving(true);
    const payload: any = { name: editForm.name, username: editForm.username, email: editForm.email, adminRole: editForm.adminRole };
    if (editForm.password) payload.password = editForm.password;
    const { ok, data } = await patchJson(`/api/admin/users/${editing.id}`, payload);
    setEditSaving(false);
    show(ok ? "تم تحديث بيانات الموظف" : data?.error || "فشل التحديث", ok);
...
<<<<<<< HEAD
      ...
      {/* Edit modal */}
      {editing && (
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/50 p-4" onClick={() => !editSaving && setEditing(null)}>
          <Card className="w-full max-w-lg shadow-2xl" onClick={e => e.stopPropagation()}>
            <CardHeader className="border-b border-border">
              <div className="flex items-center justify-between">
                <CardTitle className="text-base">تعديل الموظف: {editing.name}</CardTitle>
                <Button variant="ghost" size="icon" onClick={() => !editSaving && setEditing(null)}><X size={18}/></Button>
              </div>
            </CardHeader>
            <CardContent className="p-5 grid md:grid-cols-2 gap-4">
              <Input placeholder="الاسم الكامل" value={editForm.name} onChange={e=>setEditForm({...editForm,name:e.target.value})} />
              <Input placeholder="اسم المستخدم" value={editForm.username} onChange={e=>setEditForm({...editForm,username:e.target.value})} />
              <Input type="email" dir="ltr" className="text-right" placeholder="البريد الإلكتروني" value={editForm.email} onChange={e=>setEditForm({...editForm,email:e.target.value})} />
              <select className="border border-input bg-background rounded-md px-3 py-2 text-sm" value={editForm.adminRole} onChange={e=>setEditForm({...editForm,adminRole:e.target.value})}>
                <option value="support">دعم فني</option><option value="general">مشرف عام</option><option value="finance">مالي</option>
              </select>
              <Input type="password" dir="ltr" className="text-right md:col-span-2" placeholder="كلمة مرور جديدة (اختياري)" value={editForm.password} onChange={e=>setEditForm({...editForm,password:e.target.value})} />
              <div className="md:col-span-2 flex justify-end gap-3 pt-2">
                <Button variant="outline" onClick={() => setEditing(null)} disabled={editSaving}>إلغاء</Button>
                <Button variant="brand" onClick={saveEdit} disabled={editSaving}>{editSaving ? "جارٍ الحفظ..." : "حفظ"}</Button>
              </div>
            </CardContent>
          </Card>
        </div>
      ...
      <div className="flex items-center justify-between flex-wrap gap-3">
        <div>
          <h1 className="text-2xl font-bold text-foreground">المشرفين والموظفين</h1>
          <p className="text-sm text-muted-foreground mt-1">إدارة فريق العمل وصلاحياتهم — حساب المالك مثبّت ولا يمكن حذفه</p>
        </div>
        <Button variant="brand" onClick={() => setShowAdd(!showAdd)}>
          {showAdd ? <><X size={16}/> إلغاء</> : <><Plus size={16}/> إضافة موظف</>}
        </Button>
      </div>
      ...
      {showAdd && (
        <Card className="border-2 border-dashed border-[#34cc30]/30 shadow-none">
          <CardContent className="p-6">
            <h3 className="font-bold text-foreground mb-4">موظف جديد</h3>
            <div className="grid md:grid-cols-2 gap-4">
              ...
              <select className="border border-input bg-background rounded-md px-3 py-2 text-sm md:col-span-2" value={form.adminRole} onChange={e=>setForm({...form,adminRole:e.target.value})}>
                <option value="support">دعم فني</option><option value="general">مشرف عام</option><option value="finance">مالي</option>
              </select>
            </div>
            <div className="flex justify-end gap-3 mt-4">
              <Button variant="outline" onClick={() => setShowAdd(false)}>إلغاء</Button>
              <Button variant="brand" onClick={add}><Plus size={16}/> إضافة</Button>
            </div>
          </CardContent>
        </Card>
=======
...
<<<<<<< HEAD
          ...
          <Input value={search} onChange={e=>setSearch(e.target.value)} placeholder="ابحث بالاسم أو اسم المستخدم..." className="pr-10" />
        </div>
          ...
          <Table>
            <TableHeader>
              <TableRow className="bg-muted/50">
                <TableHead className="text-right p-4">الموظف</TableHead>
                <TableHead className="text-right p-4">الدور</TableHead>
                <TableHead className="text-right p-4">آخر دخول</TableHead>
                <TableHead className="text-right p-4">إجراءات</TableHead>
              </TableRow>
            </TableHeader>
            <TableBody>
              {filtered.map(s => (
                <TableRow key={s.id} className={s.isLocked ? "bg-[#34cc30]/5" : ""}>
...
<<<<<<< HEAD
                          ...
                          {s.isLocked && <Badge className="text-[10px] bg-[#34cc30] text-white border-transparent"><Lock size={10}/> مالك</Badge>}
                        </div>
                        <div className="text-xs text-muted-foreground font-mono" dir="ltr">@{s.username || s.email?.split("@")[0]}</div>
                      </div>
                    </div>
                  </TableCell>
                  <TableCell className="p-4 text-muted-foreground">{roleLabel[s.adminRole] || "—"}</TableCell>
                  <TableCell className="p-4 text-muted-foreground">{timeAgo(s.lastLoginAt)}</TableCell>
                  <TableCell className="p-4">
                    <div className="flex gap-1">
                      <Button variant="ghost" size="icon" onClick={() => openEdit(s)} disabled={s.isLocked} className={s.isLocked ? "opacity-30" : "hover:text-[#34cc30]"}><Edit size={15}/></Button>
                      <Button variant="ghost" size="icon" onClick={() => remove(s)} disabled={s.isLocked} className={s.isLocked ? "opacity-30" : "hover:text-destructive"}><Trash2 size={15}/></Button>
                    </div>
                  </TableCell>
                </TableRow>
              ))}
...
export function VerificationsPage() {
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="flex items-center justify-between">
<<<<<<< HEAD
        <h1 className="text-2xl font-bold text-foreground">طلبات التحقق</h1>
        <Badge className="bg-red-100 text-red-700 dark:bg-red-900/30 dark:text-red-300 border-transparent px-3 py-1 text-sm">{list.length} بانتظار التوثيق</Badge>
=======
            ...
            <Card key={r.id} className="shadow-sm">
              <CardContent className="p-6">
                <div className="flex items-center justify-between mb-4">
                  ...
                  <Badge className="bg-blue-100 text-blue-700 dark:bg-blue-900/30 dark:text-blue-300 border-transparent">غير موثّق</Badge>
                </div>
                <div className="flex gap-3">
                  <Button variant="brand" size="sm" onClick={() => verify(r.id, true)}><CheckCircle size={14}/> توثيق</Button>
                  <Button variant="destructive" size="sm" onClick={async () => { const { ok } = await patchJson(`/api/admin/users/${r.id}`, { isSuspended: true, reason: "فشل التحقق" }); show(ok?"تم الإيقاف":"فشل", ok); if (ok) reload(); }}><X size={14}/> رفض وإيقاف</Button>
                </div>
              </CardContent>
            </Card>
=======
...
export function SuspendedUsersPage() {
  ...
  return (
    <div className="space-y-6">
      {node}
<<<<<<< HEAD
      <h1 className="text-2xl font-bold text-foreground">المستخدمون الموقوفون</h1>
      {loading ? <Loading /> : list.length === 0 ? <Empty icon={CheckCircle} msg="لا يوجد مستخدمون موقوفون" /> : (
        <Card className="shadow-sm overflow-hidden">
          <Table>
            <TableHeader>
              <TableRow className="bg-muted/50">
                <TableHead className="text-right p-4">المستخدم</TableHead>
                <TableHead className="text-right p-4">النوع</TableHead>
                <TableHead className="text-right p-4">السبب</TableHead>
                <TableHead className="text-right p-4">تاريخ الإيقاف</TableHead>
                <TableHead className="text-right p-4">إجراءات</TableHead>
              </TableRow>
            </TableHeader>
            <TableBody>
              {list.map(s => (
                <TableRow key={s.id}>
                  <TableCell className="p-4 font-medium text-foreground">{s.name}</TableCell>
                  <TableCell className="p-4 text-muted-foreground">{s.role === "freelancer" ? "مستقل" : s.role === "client" ? "عميل" : "موظف"}</TableCell>
                  <TableCell className="p-4 text-red-600">{s.suspensionReason || "—"}</TableCell>
                  <TableCell className="p-4 text-muted-foreground">{dateAr(s.suspendedAt)}</TableCell>
                  <TableCell className="p-4"><Button variant="brand" size="sm" onClick={() => unsuspend(s.id)}>إلغاء الإيقاف</Button></TableCell>
                </TableRow>
              ))}
            </TableBody>
          </Table>
...
export function PublishedServicesPage() {
  ...
  return (
    <div className="space-y-6">
      {node}
<<<<<<< HEAD
      <h1 className="text-2xl font-bold text-foreground">الخدمات المنشورة</h1>
      {loading ? <Loading /> : (
        <Card className="shadow-sm overflow-hidden">
          <Table>
            <TableHeader>
              <TableRow className="bg-muted/50">
                ...
                <TableHead className="text-right p-4">المستقل</TableHead>
                <TableHead className="text-right p-4">السعر</TableHead>
                <TableHead className="text-right p-4">الطلبات</TableHead>
                <TableHead className="text-right p-4">التصنيف</TableHead>
                <TableHead className="text-right p-4">إجراءات</TableHead>
              </TableRow>
            </TableHeader>
            <TableBody>
              {list.map(s => (
                <TableRow key={s.id}>
                  <TableCell className="p-4 font-medium text-foreground">{s.title}</TableCell>
                  <TableCell className="p-4 text-muted-foreground">{s.freelancerName}</TableCell>
                  <TableCell className="p-4">{fmt(s.price)} ر.س</TableCell>
                  <TableCell className="p-4">{s.ordersCount}</TableCell>
                  <TableCell className="p-4"><Badge variant="secondary">{s.categoryName}</Badge></TableCell>
                  <TableCell className="p-4">
                    <Button variant="ghost" size="icon" className="hover:text-destructive" onClick={() => suspend(s.id)} title="إيقاف"><Ban size={15}/></Button>
                  </TableCell>
                </TableRow>
              ))}
            </TableBody>
...
export function PendingServicesPage() {
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="flex items-center justify-between">
<<<<<<< HEAD
        <h1 className="text-2xl font-bold text-foreground">خدمات بانتظار الموافقة</h1>
        <Badge className="bg-yellow-100 text-yellow-700 dark:bg-yellow-900/30 dark:text-yellow-300 border-transparent px-3 py-1 text-sm">{list.length} خدمة</Badge>
=======
              ...
              <CardContent className="p-6">
                <div className="flex items-center justify-between mb-3">
                  <div>
                    ...
                    <div className="text-sm text-muted-foreground">{p.freelancerName} • {p.categoryName} • {fmt(p.price)} ر.س</div>
                  </div>
                  <span className="text-sm text-muted-foreground">{dateAr(p.createdAt)}</span>
                </div>
                {p.description && <p className="text-sm text-muted-foreground mb-4">{p.description}</p>}
                <div className="flex gap-3">
                  <Button variant="brand" size="sm" onClick={() => moderate(p.id, "approve")}><CheckCircle size={14}/> موافقة</Button>
                  <Button variant="destructive" size="sm" onClick={() => moderate(p.id, "reject")}><X size={14}/> رفض</Button>
                </div>
              </CardContent>
            </Card>
=======
...
<<<<<<< HEAD
        ...
        <Button variant="brand" onClick={() => setShowAdd(!showAdd)}>
          {showAdd ? <><X size={16}/> إلغاء</> : <><Plus size={16}/> تصنيف جديد</>}
        </Button>
      </div>
      {showAdd && (
        <Card className="border-2 border-dashed border-[#34cc30]/30 shadow-none">
          <CardContent className="p-6 flex gap-3 items-end">
            <div className="flex-1">
              <label className="block text-sm text-muted-foreground mb-1">الاسم</label>
              <Input value={form.nameAr} onChange={e => setForm({...form, nameAr: e.target.value})} />
            </div>
            <div>
              <label className="block text-sm text-muted-foreground mb-1">الأيقونة</label>
              <Input value={form.icon} onChange={e => setForm({...form, icon: e.target.value})} className="w-20 text-center" />
            </div>
            <Button variant="brand" onClick={add}>حفظ</Button>
          </CardContent>
        </Card>
=======
            ...
            <Card key={c.id} className="shadow-sm hover:shadow-md transition-shadow">
              <CardContent className="p-6">
                <div className="flex items-center justify-between mb-4">
                  ...
                  <div className="flex gap-1">
                    <Button variant="ghost" size="icon" onClick={() => rename(c)} className="hover:text-[#34cc30]"><Edit size={15}/></Button>
                    <Button variant="ghost" size="icon" onClick={() => remove(c)} className="hover:text-destructive"><Trash2 size={15}/></Button>
                  </div>
                </div>
                <h3 className="font-bold text-foreground mb-1">{c.nameAr}</h3>
                <p className="text-sm text-muted-foreground">{c.servicesCount} خدمة</p>
              </CardContent>
            </Card>
=======
...
export function KeywordsPage() {
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="flex items-center justify-between">
<<<<<<< HEAD
        <h1 className="text-2xl font-bold text-foreground">الكلمات المفتاحية</h1>
        <div className="flex gap-2">
          <Input value={w} onChange={e=>setW(e.target.value)} onKeyDown={e=>e.key==="Enter"&&add()} placeholder="كلمة جديدة..." className="w-48" />
          <Button variant="brand" onClick={add}><Plus size={16}/> إضافة</Button>
        </div>
      </div>
      {loading ? <Loading /> : (
            ...
            <div className="flex flex-wrap gap-3">
              {list.map(k => (
                <span key={k.id} className="bg-[#485869]/10 dark:bg-white/10 text-[#485869] dark:text-white px-4 py-2 rounded-full text-sm font-medium flex items-center gap-2">
                  {k.word} <span className="text-xs text-muted-foreground">{k.searches}</span>
                </span>
              ))}
              {list.length === 0 && <p className="text-muted-foreground text-sm">لا توجد كلمات بعد</p>}
            </div>
          </CardContent>
        </Card>
=======
...
export function AllOrdersPage() {
  ...
  return (
    <div className="space-y-6">
<<<<<<< HEAD
      <h1 className="text-2xl font-bold text-foreground">كل الطلبات</h1>
      {loading ? <Loading /> : (
        <Card className="shadow-sm overflow-hidden">
          <Table>
            <TableHeader>
            ...
            <TableBody>
              {list.map((o:any) => (
                <TableRow key={o.id}>
                  <TableCell className="p-4 font-medium text-foreground">#ORD-{String(o.id).padStart(4,"0")}</TableCell>
                  <TableCell className="p-4">{o.serviceTitle}</TableCell>
                  <TableCell className="p-4">{o.freelancerName}</TableCell>
                  <TableCell className="p-4 text-muted-foreground">{o.clientName}</TableCell>
                  <TableCell className="p-4 font-medium">{fmt(o.amount)} ر.س</TableCell>
                  <TableCell className="p-4"><Badge className={orderStatusClass[o.status]}>{orderStatusLabel[o.status]}</Badge></TableCell>
                  <TableCell className="p-4 text-muted-foreground">{dateAr(o.createdAt)}</TableCell>
                </TableRow>
              ))}
            </TableBody>
          </Table>
...
export function ActiveOrdersPage() {
  ...
  return (
    <div className="space-y-6">
<<<<<<< HEAD
      <h1 className="text-2xl font-bold text-foreground">الطلبات النشطة</h1>
      <div className="grid md:grid-cols-4 gap-4">
        {[
          {l:"قيد التنفيذ",v:counts.active,c:"bg-blue-500"},
          {l:"بانتظار الدفع",v:counts.pending,c:"bg-yellow-500"},
          {l:"مكتملة",v:counts.completed,c:"bg-[#34cc30]"},
          {l:"في نزاع",v:counts.disputed,c:"bg-red-500"}
        ].map(s => (
          <Card key={s.l} className="shadow-sm">
            <CardContent className="p-4">
              <div className="flex items-center gap-3 mb-2"><div className={`w-3 h-3 rounded-full ${s.c}`}/><span className="text-sm text-muted-foreground">{s.l}</span></div>
          ...
          <Table>
            <TableHeader>
              <TableRow className="bg-muted/50">
                ...
                <TableHead className="text-right p-4">المستقل</TableHead>
                <TableHead className="text-right p-4">المبلغ</TableHead>
                <TableHead className="text-right p-4">التسليم</TableHead>
              </TableRow>
            </TableHeader>
            <TableBody>
              {active.map((o:any) => (
                <TableRow key={o.id}>
                  <TableCell className="p-4 font-medium">#ORD-{String(o.id).padStart(4,"0")}</TableCell>
                  <TableCell className="p-4">{o.serviceTitle}</TableCell>
                  <TableCell className="p-4">{o.freelancerName}</TableCell>
                  <TableCell className="p-4 font-medium">{fmt(o.amount)} ر.س</TableCell>
                  <TableCell className="p-4 text-muted-foreground">{dateAr(o.dueDate)}</TableCell>
                </TableRow>
              ))}
            </TableBody>
          </Table>
...
export function DisputesPage() {
  ...
  return (
    <div className="space-y-6">
      {node}
<<<<<<< HEAD
      <h1 className="text-2xl font-bold text-foreground">الطلبات في نزاع</h1>
                ...
                <div className="flex items-start justify-between mb-4">
                  <div>
                    <div className="flex items-center gap-3 mb-1">
                      <span className="font-bold text-foreground">#ORD-{String(d.id).padStart(4,"0")}</span>
                      <Badge className="bg-red-100 text-red-700 dark:bg-red-900/30 dark:text-red-300 border-transparent">في نزاع</Badge>
                    </div>
                    <div className="text-sm text-muted-foreground">{d.serviceTitle}</div>
                  </div>
                  <span className="font-bold text-foreground">{fmt(d.amount)} ر.س</span>
                </div>
                <div className="grid grid-cols-3 gap-4 text-sm mb-4">
                  <div><span className="text-muted-foreground">المستقل: </span><span className="font-medium">{d.freelancerName}</span></div>
                  <div><span className="text-muted-foreground">العميل: </span><span className="font-medium">{d.clientName}</span></div>
                  <div><span className="text-muted-foreground">التاريخ: </span><span className="font-medium">{dateAr(d.createdAt)}</span></div>
                </div>
                <div className="flex gap-2">
                  <Button variant="brand" size="sm" onClick={() => resolve(d.id,"freelancer")}>حل لصالح المستقل</Button>
                  <Button size="sm" className="bg-blue-500 hover:bg-blue-600 text-white" onClick={() => resolve(d.id,"client")}>حل لصالح العميل</Button>
                </div>
              </CardContent>
            </Card>
=======
...
export function CancelledOrdersPage() {
  ...
  return (
    <div className="space-y-6">
<<<<<<< HEAD
      <h1 className="text-2xl font-bold text-foreground">الطلبات الملغاة</h1>
        ...
        <Card className="shadow-sm overflow-hidden">
          <Table>
            ...
            <TableBody>
              {list.map((c:any) => (
                <TableRow key={c.id}>
                  <TableCell className="p-4 font-medium">#ORD-{String(c.id).padStart(4,"0")}</TableCell>
                  <TableCell className="p-4">{c.serviceTitle}</TableCell>
                  <TableCell className="p-4">{c.clientName}</TableCell>
                  <TableCell className="p-4 text-muted-foreground">{dateAr(c.createdAt)}</TableCell>
                  <TableCell className="p-4">{fmt(c.amount)} ر.س</TableCell>
                </TableRow>
              ))}
            </TableBody>
          </Table>
...
export function WalletsPage() {
  ...
  return (
    <div className="space-y-6">
<<<<<<< HEAD
      <h1 className="text-2xl font-bold text-foreground">المحافظ والأرصدة</h1>
=======
            ...
            <Table>
              <TableHeader>
                <TableRow className="bg-muted/50">
                  <TableHead className="text-right p-4">المستخدم</TableHead>
                  <TableHead className="text-right p-4">الدور</TableHead>
                  <TableHead className="text-right p-4">الرصيد</TableHead>
                  <TableHead className="text-right p-4">معلّق</TableHead>
                  <TableHead className="text-right p-4">إجمالي ما تم كسبه</TableHead>
                </TableRow>
              </TableHeader>
              <TableBody>
                {(w.data||[]).map((row:any) => (
                  <TableRow key={row.id}>
                    ...
                    <TableCell className="p-4 text-muted-foreground">{row.role}</TableCell>
                    <TableCell className="p-4 font-bold text-[#34cc30]">{fmt(row.balance)} ر.س</TableCell>
                    <TableCell className="p-4 text-yellow-600">{fmt(row.pending)} ر.س</TableCell>
                    <TableCell className="p-4">{fmt(row.totalEarned)} ر.س</TableCell>
                  </TableRow>
                ))}
              </TableBody>
            </Table>
...
>>>>>>> 8eb51cc93e5b4ccff945f13b2cd31105415d835a
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="flex items-center justify-between">
<<<<<<< HEAD
        <h1 className="text-2xl font-bold text-foreground">طلبات السحب</h1>
        <Badge className="bg-yellow-100 text-yellow-700 dark:bg-yellow-900/30 dark:text-yellow-300 border-transparent px-3 py-1 text-sm">
          {list.filter(w=>w.status==="pending").length} بانتظار
        </Badge>
      </div>
      {loading ? <Loading /> : (
        <Card className="shadow-sm overflow-hidden">
          <Table>
            <TableHeader>
              <TableRow className="bg-muted/50">
                ...
                <TableHead className="text-right p-4">آيبان</TableHead>
                <TableHead className="text-right p-4">التاريخ</TableHead>
                <TableHead className="text-right p-4">الحالة</TableHead>
            ...
            <TableBody>
              {list.map((w:any) => (
                <TableRow key={w.id}>
                  <TableCell className="p-4 font-medium">WD-{String(w.id).padStart(4,"0")}</TableCell>
                  <TableCell className="p-4">{w.freelancerName}</TableCell>
                  <TableCell className="p-4 font-bold">{fmt(w.amount)} ر.س</TableCell>
                  <TableCell className="p-4 text-muted-foreground">{w.bankName}</TableCell>
                  <TableCell className="p-4 text-xs font-mono" dir="ltr">{w.accountNumber}</TableCell>
                  <TableCell className="p-4 text-muted-foreground">{dateAr(w.createdAt)}</TableCell>
                  <TableCell className="p-4"><Badge className={statusClass[w.status]}>{statusLabel[w.status]}</Badge></TableCell>
                  <TableCell className="p-4">
                    {w.status === "pending" ? (
                      <div className="flex gap-1">
                        <Button variant="brand" size="sm" onClick={() => action(w.id,"approved")}>تأكيد</Button>
                        <Button variant="destructive" size="sm" onClick={() => action(w.id,"rejected")}>رفض</Button>
                      </div>
...
<<<<<<< HEAD
  const statusClass: any = {
    paid:    "bg-green-100 text-green-700 dark:bg-green-900/30 dark:text-green-300 border-transparent",
    pending: "bg-yellow-100 text-yellow-700 dark:bg-yellow-900/30 dark:text-yellow-300 border-transparent",
    failed:  "bg-red-100 text-red-700 dark:bg-red-900/30 dark:text-red-300 border-transparent",
  };
...
<<<<<<< HEAD
              ...
              <Input value={search} onChange={e => setSearch(e.target.value)} placeholder="بحث بالرقم أو الاسم..." className="pr-9" />
            </div>
            <div className="flex gap-1">
              {["الكل","paid","pending","failed"].map(v => (
                <Button key={v} size="sm" variant={statusF===v ? "brand" : "outline"} onClick={() => setStatusF(v)}>
            ...
            <Table>
              <TableHeader>
                <TableRow className="bg-muted/50">
                  ...
                  <TableHead className="text-right p-4">المستقل</TableHead>
                  <TableHead className="text-right p-4">المبلغ</TableHead>
                  <TableHead className="text-right p-4">عمولة المنصة</TableHead>
                  <TableHead className="text-right p-4">التاريخ</TableHead>
                  <TableHead className="text-right p-4">الحالة</TableHead>
                </TableRow>
              </TableHeader>
              <TableBody>
                {filtered.map((o: any) => (
                  <TableRow key={o.id}>
                    <TableCell className="p-4 font-medium text-foreground">#{o.publicCode || String(o.id).padStart(4,"0")}</TableCell>
                    <TableCell className="p-4">{o.clientName || o.clientPhone || "—"}</TableCell>
                    <TableCell className="p-4">{o.freelancerName || "—"}</TableCell>
                    <TableCell className="p-4 font-bold">{fmt(o.paidAmount || o.amount)} ر.س</TableCell>
                    <TableCell className="p-4 text-[#34cc30]">{fmt(o.platformFee)} ر.س</TableCell>
                    <TableCell className="p-4 text-muted-foreground">{dateAr(o.createdAt)}</TableCell>
                    <TableCell className="p-4"><Badge className={statusClass[o.paymentStatus] || "bg-gray-100 text-gray-600 border-transparent"}>{statusLabel[o.paymentStatus] || o.paymentStatus || "—"}</Badge></TableCell>
                  </TableRow>
                ))}
                {filtered.length === 0 && <TableRow><TableCell colSpan={7} className="p-12 text-center text-muted-foreground">لا توجد معاملات</TableCell></TableRow>}
              </TableBody>
            </Table>
          </Card>
=======
          ...
          <Table>
            <TableHeader>
              <TableRow className="bg-muted/50">
                ...
                <TableHead className="text-right p-4">المستقل</TableHead>
                <TableHead className="text-right p-4">المبلغ</TableHead>
                <TableHead className="text-right p-4">عمولة</TableHead>
            ...
            <TableBody>
              {list.map((o: any, i: number) => (
                <TableRow key={o.id}>
                  <TableCell className="p-4 font-medium">INV-{String(i+1).padStart(4,"0")}</TableCell>
                  <TableCell className="p-4">{o.clientName || o.clientPhone || "—"}</TableCell>
                  <TableCell className="p-4">{o.freelancerName || "—"}</TableCell>
                  <TableCell className="p-4 font-bold">{fmt(o.paidAmount || o.amount)} ر.س</TableCell>
                  <TableCell className="p-4 text-[#34cc30]">{fmt(o.platformFee)} ر.س</TableCell>
                  <TableCell className="p-4">{fmt(Number(o.paidAmount || o.amount) - Number(o.platformFee))} ر.س</TableCell>
                  <TableCell className="p-4 text-muted-foreground">{dateAr(o.completedAt || o.createdAt)}</TableCell>
                </TableRow>
              ))}
              {list.length === 0 && <TableRow><TableCell colSpan={7} className="p-12 text-center text-muted-foreground">لا توجد فواتير مدفوعة بعد</TableCell></TableRow>}
            </TableBody>
          </Table>
        </Card>
=======
...
<<<<<<< HEAD
  ...
  return (
    <div className="space-y-6">
      <h1 className="text-2xl font-bold text-foreground">روابط الدفع</h1>
      {loading ? <Loading /> : (
        <Card className="shadow-sm overflow-hidden">
          <Table>
            <TableHeader>
              <TableRow className="bg-muted/50">
                <TableHead className="text-right p-4">رقم الطلب</TableHead>
                <TableHead className="text-right p-4">جوال العميل</TableHead>
                <TableHead className="text-right p-4">المبلغ</TableHead>
                <TableHead className="text-right p-4">الحالة</TableHead>
                <TableHead className="text-right p-4">انتهاء الصلاحية</TableHead>
                <TableHead className="text-right p-4">تاريخ الإنشاء</TableHead>
              </TableRow>
            </TableHeader>
            <TableBody>
              {sessions.map((ps: any) => (
                <TableRow key={ps.id}>
                  <TableCell className="p-4 font-medium">{ps.orderId ? `#${ps.orderId}` : "—"}</TableCell>
                  <TableCell className="p-4 font-mono text-xs" dir="ltr">{ps.clientPhone}</TableCell>
                  <TableCell className="p-4 font-bold">{fmt(ps.amount)} ر.س</TableCell>
                  <TableCell className="p-4"><Badge className={statusClass[ps.status] || "bg-gray-100 text-gray-600 border-transparent"}>{statusLabel[ps.status] || ps.status}</Badge></TableCell>
                  <TableCell className="p-4 text-muted-foreground text-xs">{ps.expiresAt ? dateAr(ps.expiresAt) : "—"}</TableCell>
                  <TableCell className="p-4 text-muted-foreground text-xs">{dateAr(ps.createdAt)}</TableCell>
                </TableRow>
              ))}
              {sessions.length === 0 && <TableRow><TableCell colSpan={6} className="p-12 text-center text-muted-foreground">لا توجد روابط دفع بعد</TableCell></TableRow>}
            </TableBody>
          </Table>
        </Card>
=======
