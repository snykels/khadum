 ...
 *
<<<<<<< HEAD
 * GET  — verification handshake using WHATSAPP_VERIFY_TOKEN (env) or
 *         whatsapp.verifyToken (DB settings via admin panel)
 * POST — inbound messages, signature verified by WHATSAPP_APP_SECRET (env)
 *         or whatsapp.appSecret (DB settings via admin panel)
 *
 * DB settings take priority over env vars, enabling configuration without
 * deployment via the admin panel at /admin/messaging.
=======
...
<<<<<<< HEAD
  // Load from DB first, fall back to env var
  const cfg = await loadSettings("whatsapp");
  const expected = cfg.verifyToken || process.env.WHATSAPP_VERIFY_TOKEN;
...
function verifySignature(rawBody: string, signature: string | null, appSecret: string | undefined): boolean {
  ...
  if (!appSecret) {
    ...
    console.warn("[whatsapp/webhook] app secret not set — skipping signature check (dev only)");
=======
...
<<<<<<< HEAD
  // Load app secret from DB first, fall back to env var
  const cfg = await loadSettings("whatsapp");
  const appSecret = cfg.appSecret || process.env.WHATSAPP_APP_SECRET;
