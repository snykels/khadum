...
import { classifyDeliveryIntent, handleFreelancerDelivery, handleClientDeliveryResponse, handleRatingResponse } from "./delivery";
...
export async function handleInbound(msg: InboundMessage): Promise<void> {
  ...
  // ─── فحص التقييم ─────────────────────────────────────────────────────────
  // Client in "rating" stage after order completion — capture 1–5 rating.
  const ratingCtx = earlySession.context as {
    awaiting_rating?: boolean;
    rating_order_id?: number;
    rating_freelancer_id?: number | null;
  };
  if (ratingCtx.awaiting_rating && ratingCtx.rating_order_id) {
    await handleRatingResponse(
      phone,
      msg.text,
      ratingCtx.rating_order_id,
      ratingCtx.rating_freelancer_id ?? null,
    );
    return;
  }
  ...
  if (earlySession.stage === "delivery" && deliveryCtx.awaiting_delivery_confirm && deliveryCtx.delivery_order_id) {
    ...
    if (deliveryResult === "accepted") {
      // completeOrder() already sent completion + rating prompt — just return.
      return;
    }
